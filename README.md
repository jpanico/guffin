...
export-roam-tree <page_title_or_node_uid> --port <port> --graph <graph> --token <token> --output-dir <output_dir> \
  [--format markdown|pdf|epub] [--type default|book|manuscript] [--bundle|--no-bundle] \
  ...
  [--daily-note-format roam-long|iso|abbrev-month-day]
```

The output filename stem embeds the project type: `<target>.<type>.<ext>` (e.g. `Foo.default.epub`, `Foo.book.pdf`), so the same target exported under different types lands in distinct files.
...
# Plain .md — creates ~/docs/Test_Article.default.md
...
`--type default|book|manuscript` (default `default`, short `-T`) selects the **project profile** — the kind of work being produced — independently of `--format` (see [docs/render-pipeline.md](docs/render-pipeline.md)). The profile resolves to five structural directives that are applied across the paginated formats:
