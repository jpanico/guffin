...
import pytest
from conftest import article1_vertex_tree, article8_vertex_tree, asset_storage
...
class TestVertexTreeToPandocPdfVertex:
    ...
    def _tree(self, source: HttpUrl = _PDF_URL) -> VertexTree:
        ...
        pdf = PdfVertex(uid="pdf00001a", storage=asset_storage(source))
    ...
    def test_link_label_prefers_file_name(self) -> None:
        """When the originally uploaded filename is known, it labels the link."""
        page = PageVertex(uid="page00001", title="P", children=["pdf00001a"])
        pdf = PdfVertex(uid="pdf00001a", storage=asset_storage(_PDF_URL), file_name="dummy.pdf")
    ...
    def test_inline_placed_pdf_stays_structural(self) -> None:
        ...
        page = PageVertex(uid="page00001", title="P", children=["pdf00001a"])
        pdf = PdfVertex(uid="pdf00001a", storage=asset_storage(_PDF_URL), attribute_assignments=[inline_tag])
...
# ---------------------------------------------------------------------------
# TestPdfPlacementStamping
# ---------------------------------------------------------------------------


def _pdf_render_tag(value: str) -> AttributeAssignment:
...
def _pdf_site(uid: str, target_uid: str, render: str | None = None) -> TextVertex:
    """A text vertex whose entire text is a standalone vertex link to *target_uid*, optionally tagged."""
    return TextVertex(
        uid=uid,
        text=f"[paper.pdf](x-guffin:vertex/{target_uid})",
        attribute_assignments=[_pdf_render_tag(render)] if render else None,
    )


def _stamped_placements(doc: pf.Doc) -> list[str]:
    """Collect every Link's PDF-placement stamp in document order."""
    found: list[str] = []

    def _action(elem: pf.Element, doc: pf.Doc) -> None:
        if isinstance(elem, pf.Link) and PDF_PLACEMENT_ATTRIBUTE in elem.attributes:
            found.append(elem.attributes[PDF_PLACEMENT_ATTRIBUTE])
        return None
...
class TestPdfPlacementStamping:
    """vertex_tree_to_pandoc() stamps each PDF display occurrence's resolved pdf-render placement."""

    @staticmethod
    def _direct_tree(inline: bool) -> VertexTree:
        """A page whose child is a PDF embed, optionally tagged pdf-render:: inline."""
        page = PageVertex(uid="page00001", title="P", children=["pdf00001a"])
        pdf = PdfVertex(
            uid="pdf00001a",
            storage=asset_storage(_PDF_URL),
            attribute_assignments=[_pdf_render_tag("inline-native")] if inline else None,
        )
        return VertexTree(tree_vertices=[page, pdf])

    @staticmethod
    def _ref_tree(site_render: str | None, target_inline: bool) -> VertexTree:
        """A page whose child standalone-references a PDF, site and target optionally tagged."""
    ...
    def test_sites_of_one_pdf_stamp_independently(self) -> None:
        ...
        pdf = PdfVertex(uid="pdf00001a", storage=asset_storage(_PDF_URL))
        tree = VertexTree(
            tree_vertices=[
                page,
    ...
    def test_strip_pdf_placement_removes_scaffold(self) -> None:
        ...
        assert _stamped_placements(doc) == []
...
class TestBuildBlocksCoalescing:
    ...
    def test_link_placed_pdf_coalesces_with_text_siblings(self) -> None:
        ...
        pdf = PdfVertex(uid="pdf000001", storage=asset_storage(_PDF_URL))
        blocks = build_child_blocks(
            ["txt000001", "pdf000001"],
...
class TestVertexTreeToPandocArticleFixture:
    ...
    def test_block_count(self, doc: pf.Doc) -> None:
        ...
        # 1 Div(callout) + 3 H1s + 6 H2s + 4 H3s + 1 H4 + 3 Para(Link) + 7 BulletList + 1 Table = 26
        ...
        assert len(list(doc.content)) == 26
