...
from guffin.common.github_file_ref import (
    ...
    GitHubFileRef,
    blob_github_url,
    parse_blob_github_url,
    raw_github_url,
    verified_blob_github_url,
    verified_commit_sha,
)
...
class TestBlobGitHubUrlType:
    """BlobGitHubUrl and its imperative form certify the full URL grammar, keeping the text verbatim."""

    _ADAPTER = TypeAdapter[BlobGitHubUrl](BlobGitHubUrl)
    _URL = "https://github.com/psf/requests/blob/main/setup.py#L1-L5"

    def test_verified_url_passes_through(self) -> None:
        """A parseable blob URL is returned unchanged."""
        assert verified_blob_github_url(self._URL) == self._URL

    def test_verified_rejects_wrong_host(self) -> None:
        """The imperative form applies the full parse, not a shape check."""
        with pytest.raises(ValueError, match="github.com"):
            verified_blob_github_url("https://raw.githubusercontent.com/psf/requests/main/setup.py")
    ...
    def test_annotation_enforces_at_type_boundary(self) -> None:
        ...
        with pytest.raises(ValidationError, match="query string"):
            self._ADAPTER.validate_python("https://github.com/psf/requests/blob/main/setup.py?x=1")


class TestParseBlobGithubUrl:
    """parse_blob_github_url() decomposes every supported blob-URL form and rejects everything else."""

    def test_bare_branch_ref(self) -> None:
        """A single-segment branch ref parses into its four parts."""
        file_ref = parse_blob_github_url("https://github.com/psf/requests/blob/main/src/requests/api.py")
    ...
    def test_qualified_branch_ref(self) -> None:
        ...
        file_ref = parse_blob_github_url("https://github.com/psf/requests/blob/refs/heads/main/setup.py")
    ...
    def test_qualified_tag_ref(self) -> None:
        ...
        file_ref = parse_blob_github_url("https://github.com/psf/requests/blob/refs/tags/v2.32.3/setup.py")
    ...
    def test_sha_ref(self) -> None:
        ...
        file_ref = parse_blob_github_url(f"https://github.com/psf/requests/blob/{_SHA}/setup.py")
    ...
    def test_single_line_fragment(self) -> None:
        ...
        file_ref = parse_blob_github_url("https://github.com/psf/requests/blob/main/setup.py#L12")
    ...
    def test_line_range_fragment(self) -> None:
        ...
        file_ref = parse_blob_github_url("https://github.com/psf/requests/blob/main/setup.py#L10-L40")
        assert file_ref.line_range == LineRange(start=10, end=40)

    def test_rejects_wrong_host(self) -> None:
        """A raw.githubusercontent.com (non-blob) URL is rejected."""
        with pytest.raises(ValueError, match="github.com"):
            parse_blob_github_url("https://raw.githubusercontent.com/psf/requests/main/setup.py")

    def test_rejects_http_scheme(self) -> None:
        """A plain-http URL is rejected."""
        with pytest.raises(ValueError, match="https"):
            parse_blob_github_url("http://github.com/psf/requests/blob/main/setup.py")

    def test_rejects_query_string(self) -> None:
        """A query string (e.g. a cache-buster) is not part of the canonical reference."""
        with pytest.raises(ValueError, match="query string"):
            parse_blob_github_url("https://github.com/psf/requests/blob/main/setup.py?token=x")

    def test_rejects_missing_blob_segment(self) -> None:
        """A github.com URL without the blob segment (e.g. a tree page) is rejected."""
        with pytest.raises(ValueError, match="'blob'"):
            parse_blob_github_url("https://github.com/psf/requests/tree/main/src/requests")

    def test_rejects_missing_path(self) -> None:
        """A URL with no file path after the ref is rejected."""
        with pytest.raises(ValueError, match="file-path"):
            parse_blob_github_url("https://github.com/psf/requests/blob/main")
    ...
    def test_rejects_qualified_ref_without_path(self) -> None:
        ...
        with pytest.raises(ValueError, match="no file path"):
            parse_blob_github_url("https://github.com/psf/requests/blob/refs/heads/main")

    def test_rejects_empty_segment(self) -> None:
        """A doubled slash yields an empty segment, which is rejected."""
        with pytest.raises(ValueError, match="file-path"):
            parse_blob_github_url("https://github.com/psf//blob/main/setup.py")
    ...
    def test_rejects_malformed_fragment(self) -> None:
        ...
        with pytest.raises(ValueError, match="not a line range"):
            parse_blob_github_url("https://github.com/psf/requests/blob/main/setup.py#readme")


class TestRawGithubUrl:
    """raw_github_url() derives the raw-content URL, optionally re-pinned at a commit."""
...
class TestBlobGithubUrl:
    ...
    def test_round_trips_through_parse(self) -> None:
        """parse_blob_github_url(blob_github_url(ref)) reproduces the reference, range included."""
        ranged = self._FILE_REF.model_copy(update={"line_range": LineRange(start=10, end=40)})
        assert parse_blob_github_url(blob_github_url(ranged)) == ranged
