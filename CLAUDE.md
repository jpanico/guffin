...
export-roam-tree <page_title_or_node_uid> -p <port> -g <graph> -t <token> -o <output_dir> [--format markdown|pdf|epub] [--type default|book|manuscript] [--bundle|--no-bundle] [--cache-dir <dir>] [--template-dir <dir>] [--suppress-attributes] [--colophon|--no-colophon] [--preamble|--no-preamble] [--numbering|--no-numbering] [--daily-note-format roam-long|iso]
...
# --daily-note-format (all formats): how a reference/link to a Roam daily-note page renders its date — roam-long (default; the page's own verbose title, e.g. January 1st, 2026, so output is unchanged) or iso (ISO 8601, e.g. 2026-01-01)
...
- `GUFFIN_DAILY_NOTE_FORMAT` — backs the `--daily-note-format` flag (`export-roam-tree`, all formats); `roam-long` (default; a daily-note-page reference keeps the page's own verbose title, so output is unchanged) or `iso` (renders those references as ISO 8601 dates)
