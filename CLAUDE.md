...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary: `GuffinAttribute` (StrEnum of the attributes Guffin recognizes — `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER` — all in the `AttributeDomain.GUFFIN` domain, which each member carries via the `DOMAIN` class-constant, an `enum.nonmember`)
