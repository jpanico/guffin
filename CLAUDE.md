...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `publishing_semantics.py` — Guffin's own attribute vocabulary (format-independent, publishing-standard): `Anchor` (StrEnum `PAGE`/`HEADING`, each carrying the `VertexType` an attribute with that anchor attaches to — the single source of truth for the Anchor↔VertexType correspondence), `Matter` (StrEnum `front-matter`/`body-matter`/`back-matter` — a book division), `PublishingAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Anchor`), `PublishingSemantics` (an `enum.Enum` of `PublishingAttribute` — page-anchored document metadata `TITLE`/`SUBTITLE`/`AUTHORS`/`DATE`/`PUBLISHER`/`RIGHTS`/`IDENTIFIER` plus the heading-anchored `ELEMENT_TYPE` and `MATTER` tags; `_value_` typed as `PublishingAttribute`), `StructuralElement` (StrEnum of a book's organizational parts `TITLE_PAGE`…`COLOPHON`, each carrying its `Matter` — the values an `element-type` tag takes; no `cover` member, since per CMOS only the book interior is matter-classified (the cover is exterior, supplied as metadata / a cover image); each element's `Matter` is the *conventional* placement, aligned with the **Chicago Manual of Style (CMOS)**, independent of any output format), and the functions `element_type_of()` / `matter_of()` (read an `element-type` / `matter` assignment's value as a `StructuralElement` / `Matter`, rejecting non-members), `find_publishing_attribute()` (a vertex's assignment for a `PublishingSemantics` attribute), the tolerant per-heading readers `element_type_of_vertex()`/`matter_of_vertex()` (absent or illegal tags resolve to `None` with a warning) and `resolved_matter()` (a heading's resolved `Matter` division — a bare `matter::` tag overrides the element's conventional placement, logging any disagreement), `has_parts()` (whether any render-visible level-1 heading in a `VertexTree` — block-embed-transcluded headings included — is tagged `element-type:: part`), and the vocabulary validation pass — the `common/validation` `Validator`s `all_attributes_anchored()` (every recognized guffin attribute must sit on its anchor's vertex type — the `PublishingAttribute.anchor` invariant), `all_element_type_values_legal()` / `all_matter_values_legal()` (every `element-type` / `matter` value must be a `StructuralElement` / `Matter` member), and `all_matter_tags_at_section_level()` (a `matter` tag sits at the book's section level — level 1, or level 2 in a parts book, since that is where chapter-shaped sections live), run via `validate_semantics()` (accumulates a `ValidationResult`; `cli/common.fetch_roam_trees` runs it on every fetch — advisory warnings for `dump-roam-tree`, but `export-roam-tree` fetches with `strict_semantics=True`, so violations raise `SemanticsValidationError` and abort the export with exit 1). Sits at the **top of the `model/` conceptual stack**: it may depend on the structural primitives (`attribute.py`, `vertex.py`, `vertex_tree.py`), and none of them may depend on it
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `asset_fetch.py` — Pandoc-free asset fetching; `AssetRef` (UID + on-disk path + image pixel size, `None` for non-image assets) and `fetch_assets()` / `fetch_and_enrich_assets()` (fetch a `VertexTree`'s Cloud Firestore image and PDF assets to a local dir, returning `{uid: AssetRef}`; the enrich variant also populates each `ImageVertex.original_image_size`)
