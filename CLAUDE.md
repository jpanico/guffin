...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary (format-independent, publishing-standard): `Anchor` (StrEnum `PAGE`/`HEADING`, each carrying the `VertexType` an attribute with that anchor attaches to — the single source of truth for the Anchor↔VertexType correspondence), `Matter` (StrEnum `front-matter`/`body-matter`/`back-matter` — a book division), `GuffinAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Anchor`), `GuffinSemantics` (an `enum.Enum` of `GuffinAttribute` — page-anchored document metadata `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER` plus the heading-anchored `ELEMENT_TYPE` and `MATTER` tags; `_value_` typed as `GuffinAttribute`), `StructuralElement` (StrEnum of a book's organizational parts `COVER`…`COLOPHON`, each carrying its `Matter` — the values an `element-type` tag takes), and `element_type_of()` / `matter_of()` (read an `element-type` / `matter` assignment's value as a `StructuralElement` / `Matter`, rejecting non-members)
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `pandoc_rendering.py` — shared Pandoc/Panflute rendering utilities; `vertex_tree_to_pandoc()` builds a Panflute `Doc` from a `VertexTree` (batch-parsing inline Pandoc Markdown via a single Pandoc call); a heading's `element-type`/`matter` tags stamp `epub:type` on the section header (via `epub_type_for`) and mark non-body-matter sections `unnumbered` so Pandoc's `--number-sections` numbers only body-matter chapters (a bare `matter::` tag overrides the element's default matter, logging any disagreement)
