...
- `src/guffin/` — main package
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `asset_fetch.py` — Pandoc-free asset fetching; `AssetRef` (UID + on-disk path + image pixel size (`None` for non-image assets) + original upload filename when known) and `fetch_assets()` / `fetch_and_enrich_assets()` (fetch a `VertexTree`'s Cloud Firestore image and PDF assets to a local dir, returning `{uid: AssetRef}`; the enrich variant also populates each `ImageVertex.original_image_size` and `PdfVertex.original_file_name`)
