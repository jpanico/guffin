...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `vertex.py` — `Vertex` union and all nine concrete vertex types (`PageVertex`, `HeadingVertex`, `TextVertex`, `ImageVertex`, `CalloutVertex`, `CodeBlockVertex`, `BlockQuoteVertex`, `TableVertex`, `BlockEmbedVertex`); the shared `_BaseVertex.attribute_assignments` field holding folded Roam attribute assignments (`<attribute>:: <value>, …`); `find_attribute_assignment()`/`find_guffin_attribute()` look up a folded assignment by name + `AttributeDomain`; `VertexType`, `VertexChildren`, `VertexRefs`, `vertex_adapter`
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary: `Role` (StrEnum `PUBLISHING`/`STRUCTURAL` — the role an attribute plays), `Level` (StrEnum `DOCUMENT`/`HEADER` — the structural level an attribute applies at), `GuffinAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Role` and `Level`), and `GuffinSemantics` (an `enum.Enum` whose members are `GuffinAttribute` instances — publishing metadata `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER` (`PUBLISHING`/`DOCUMENT`) plus the structural book-section tags `COVER`…`COLOPHON`, incl. `CHAPTER`/`SECTION`/`SUB_SECTION`/`SUB_SUB_SECTION` (`STRUCTURAL`/`HEADER`); `_value_` is typed as `GuffinAttribute` so members are statically known to be `GuffinAttribute`s)
