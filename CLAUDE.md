...
- `src/guffin/` — main package
  ...
  - **`common/` sub-package** (`src/guffin/common/`) — cross-cutting helpers shared across the package
    ...
    - `revision.py` — content revision, the content-side counterpart of `provenance.py`: `Revision` model (`content_hash` — the revision's identity, a SHA-256 over the canonical transient-stripped fetched node set — plus `last_edited_at`, an advisory upper bound from Roam's edit bookkeeping, an optional authored `label`, and `fetched_at`), with `summary()` rendering one line; the capture function lives in `roam/revision.py` (it needs the raw wire response)
  ...
  - **`roam/` sub-package** (`src/guffin/roam/`) — all Roam Research data model, API, and processing modules
    ...
    - `revision.py` — captures a content `Revision` from a raw fetch: `content_hash()` (SHA-256 over the canonical form — transient keys stripped via `without_transient_keys`, rows sorted by uid, stub lists sorted by id, keys sorted — so neither wire ordering nor session state can move the digest) and `gather_revision()` (hash + max create/edit bookkeeping timestamp + caller-resolved label + capture moment)
