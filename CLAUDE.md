...
# --type (-T): project profile (default/book/manuscript); appended to the output filename stem; plumbed to all renderers (applied so far: top_level_division in both formats — EPUB split-level + PDF book mode (chapter page breaks + level-1 numbering) — and number_sections in both; remaining: title page, metadata)
...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `vertex_type.py` — `VertexType` StrEnum classifying each vertex by the shape of its source `RoamNode` (`PAGE`/`TEXT`/`HEADING`/`IMAGE`/`CALLOUT`/`CODE_BLOCK`/`BLOCK_QUOTE`/`TABLE`/`BLOCK_EMBED`); a dependency-free leaf module so both `vertex.py` and `guffin_semantics.py` can reference it without a cycle
    - `vertex.py` — `Vertex` union and all nine concrete vertex types (`PageVertex`, `HeadingVertex`, `TextVertex`, `ImageVertex`, `CalloutVertex`, `CodeBlockVertex`, `BlockQuoteVertex`, `TableVertex`, `BlockEmbedVertex`); the shared `_BaseVertex.attribute_assignments` field holding folded Roam attribute assignments (`<attribute>:: <value>, …`); `find_attribute_assignment()`/`find_guffin_attribute()` look up a folded assignment by name + `AttributeDomain`; `VertexChildren`, `VertexRefs`, `vertex_adapter` (`VertexType` is defined in `vertex_type.py` and re-exported here)
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary (format-independent, publishing-standard): `Anchor` (StrEnum `PAGE`/`HEADING`, each carrying the `VertexType` an attribute with that anchor attaches to — the single source of truth for the Anchor↔VertexType correspondence), `Matter` (StrEnum `front-matter`/`body-matter`/`back-matter` — a book division), `GuffinAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Anchor`), `GuffinSemantics` (an `enum.Enum` of `GuffinAttribute` — page-anchored document metadata `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER` plus the heading-anchored `ELEMENT_TYPE` and `MATTER` tags; `_value_` typed as `GuffinAttribute`), `StructuralElement` (StrEnum of a book's organizational parts `TITLE_PAGE`…`COLOPHON`, each carrying its `Matter` — the values an `element-type` tag takes; no `cover` member, since per CMOS only the book interior is matter-classified (the cover is exterior, supplied as metadata / a cover image); each element's `Matter` is the *conventional* placement, aligned with the **Chicago Manual of Style (CMOS)**, independent of any output format), and `element_type_of()` / `matter_of()` (read an `element-type` / `matter` assignment's value as a `StructuralElement` / `Matter`, rejecting non-members)
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `epub_semantics.py` — EPUB structural-semantics vocabulary and the model→EPUB mapping: `EpubDivision` (`frontmatter`/`bodymatter`/`backmatter`), `EpubType` (the `epub:type` terms Guffin emits, each carrying its `EpubDivision`), and `epub_type_for()` — the explicit, member-keyed `StructuralElement → EpubType` map (deliberately partial; `None` for elements with no EPUB counterpart; bridges label divergences like `table-of-contents`→`toc`). **Contrast with `StructuralElement.matter`**: that carries the *conventional* (CMOS) placement, whereas each `EpubType.division` records the `<body>` division **Pandoc assigns that term out of the box** (verified empirically against Pandoc's output) — the two intentionally diverge, and their difference (e.g. `introduction`, `prologue`, `table-of-contents`, `glossary`, `endnotes`) is the worklist for any pass that post-processes the EPUB `<body>` division back to the CMOS placement
