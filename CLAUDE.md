...
- `src/guffin/` — main package
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `pdf_rendering.py` — renders a `VertexTree` to PDF: invokes `pandoc_rendering`, serializes to Pandoc JSON, converts to PDF via Pandoc + Typst; every fetched PDF asset is attached to the output via Typst `pdf.attach` under its originally uploaded filename, and an embed tagged `pdf-render:: inline` additionally renders all of its pages in the document flow (one full-width Typst `image(page:)` per page — requires Typst ≥ 0.14), the untagged default keeping a hyperlink (`_prepare_pdf_embeds`/`_apply_pdf_embeds`, a post-build Doc rewrite of the PDF link paragraphs)
