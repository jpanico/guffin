...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary (format-independent, publishing-standard): `Anchor` (StrEnum `PAGE`/`HEADING`, each carrying the `VertexType` an attribute with that anchor attaches to — the single source of truth for the Anchor↔VertexType correspondence), `Matter` (StrEnum `front-matter`/`body-matter`/`back-matter` — a book division), `GuffinAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Anchor`), `GuffinSemantics` (an `enum.Enum` of `GuffinAttribute` — page-anchored document metadata `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER` plus the heading-anchored `ELEMENT_TYPE` tag; `_value_` typed as `GuffinAttribute`), `StructuralElement` (StrEnum of a book's organizational parts `COVER`…`COLOPHON`, each carrying its `Matter` — the values an `element-type` tag takes), and `element_type_of()` (reads an `element-type` assignment's value as a `StructuralElement`, rejecting non-members)
