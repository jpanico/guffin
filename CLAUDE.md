...
- `src/guffin/` — main package
  ...
  - **`render/` sub-package** (`src/guffin/render/`) — output rendering: turns the normalized model into consumable output (document export and terminal display) plus the configuration and bundled resources that drive it
    ...
    - `image_fetch.py` — Pandoc-free image/PDF-asset fetching; `ImageRef` (UID + on-disk path + `ImageSize`), `fetch_images()` (fetches a `VertexTree`'s Cloud Firestore image assets to a local dir, returning `{uid: ImageRef}`), and `fetch_pdfs()` (likewise for PDF assets, returning `{uid: Path}`)
