...
- `src/guffin/` — main package
  ...
  - **`model/` sub-package** (`src/guffin/model/`) — core normalized-graph model: UID primitives, vertex types, tree, presentation overlay, render bundle, and inter-vertex links
    ...
    - `guffin_semantics.py` — Guffin's own attribute vocabulary: `Role` (StrEnum `PUBLISHING`/`SEMANTICS` — the role an attribute plays), `Level` (StrEnum `DOCUMENT`/`HEADER` — the structural level an attribute applies at), `GuffinAttribute` (an `Attribute` subclass that pins `domain` to `AttributeDomain.GUFFIN` and adds a required `Role` and `Level`), and `GuffinSemantics` (an `enum.Enum` whose members — `TITLE`/`AUTHORS`/`DATE`/`IDENTIFIER`, all `PUBLISHING`/`DOCUMENT` — are `GuffinAttribute` instances; `_value_` is typed as `GuffinAttribute` so members are statically known to be `GuffinAttribute`s)
