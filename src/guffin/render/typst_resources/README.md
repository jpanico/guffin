...
| `typst_todo.lua` | a `Plain` or `Para` leading with a TODO item's checkbox glyph (`☐` U+2610 / `☒` U+2612, stamped from the vertex's `TodoState`) | a checkbox **drawn** from Typst primitives — a stroked `#box`, crossed by two `place(line(…))` strokes for a completed item — replacing the glyph, which the body font (Noto Sans) lacks and would otherwise render as tofu |
