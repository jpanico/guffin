...
| `apl.sublime-syntax` | APL highlighting grammar — Typst's built-in syntect set has no APL either; vendored from spyder-ide/pysyntect (MIT; a Sublime conversion of Alhadis/language-apl, ISC), loaded the same way via `code-syntaxes` |
...
pypandoc.convert_text(json_str, "typst", format="json", extra_args=[
    ...
    f"--lua-filter={bundled_dir / 'typst_todo.lua'}",
...
| `typst_todo.lua` | any `Str` that is a TODO checkbox glyph (`☐` U+2610 / `☒` U+2612, stamped from the vertex's `TodoState` — leading a TODO item's own blocks or inside a reference's display inlines) | a checkbox **drawn** from Typst primitives — a stroked `#box`, crossed by two `place(line(…))` strokes for a completed item — replacing the glyph, which the body font (Noto Sans) lacks and would otherwise render as tofu |
...
| `typst_caption.lua` | a `Div` with a `caption` class (the caption below an image or code listing — the block's own nested content — or the `code-source` attribution below a sourced listing) | a raw Typst block styled as a caption: `#block(above: 0.5em)[#text(size: 0.8em, fill: luma(35%))[…]]`, the caption's content serialized inside |
| `typst_quote.lua` | a `Div` with a `fancy-quote` class (its `fancy-quote-text` and optional `fancy-quote-attribution` sub-`Div`s) | a call to the template's `#fancy-quote(quote: […], attribution: […])` helper (`default_styles.typ`) — a barless pull-quote whose quotation line is set at `cfg.quote-size` (default 1.2x body size) and `cfg.quote-weight` (default bold) in `cfg.quote-font` led by an oversize `“` and whose attribution is italic in `cfg.attribution-font` |
...
- **`typst_quote.lua`** is registered **last** so any inline transforms (e.g. `typst_color_span.lua`) have already rewritten the quotation/attribution content before it is serialized to Typst (via `pandoc.write`) and marshaled into the `#fancy-quote` helper call. The helper (`default_styles.typ`) reads `cfg.quote-font` / `cfg.attribution-font` and `cfg.quote-size` / `cfg.quote-weight`, so the pull quote's fonts, size (a ratio of the body size; the oversize opening mark scales with it), and weight are set in `base_cfg.typ` and overridable via `user_cfg.typ`.
