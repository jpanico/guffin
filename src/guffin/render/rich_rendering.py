...
- :func:`build_rich_node_tree` — build a Rich :class:`~rich.tree.Tree` from a
  :class:`~guffin.roam.node_tree.NodeTree` using a depth-first traversal.
- :func:`build_rich_refs_box` — build a Rich :class:`~rich.panel.Panel` summarising the
  back-reference nodes in a :class:`~guffin.roam.node_tree.NodeTree`.
...
def build_node_panel(node: RoamNode, props: list[str] = DEFAULT_NODE_PANEL_PROPS, *, truncate: bool = True) -> Panel:
    ...
    match nt:
        ...
        case NodeType.TEXT_BLOCK | NodeType.TODO_BLOCK:
            assert node.string is not None
...
def build_rich_node_tree(
    ...
    :meth:`~guffin.roam.node_tree.NodeTree.dfs`, attaching each node as a Rich
...
def build_rich_refs_box(
    ...
    *referenced by* panel listing the ids of tree nodes that cite it on the right.
    ...
    Returns ``None`` when :attr:`~guffin.roam.node_tree.NodeTree.refs_by_id` is empty.

    Args:
        tree: The :class:`~guffin.roam.node_tree.NodeTree` whose
            :attr:`~guffin.roam.node_tree.NodeTree.refs_by_id` and
...
def _format_vertex_prop(vertex: Vertex, prop: str) -> Text | Table:
    ...
    match prop:
        ...
        case "file_name":
            if is_renderable_asset_vertex(vertex) or is_bare_asset_vertex(vertex):
                return Text(f"file_name={vertex.file_name or url_file_name(vertex.storage.location)}")
            return Text("file_name=N/A")
        case "media_type":
            if not (is_renderable_asset_vertex(vertex) or is_bare_asset_vertex(vertex)):
        ...
        case "source":
            if is_renderable_asset_vertex(vertex) or is_bare_asset_vertex(vertex):
                return Text(f"source={vertex.storage.location}")
            return Text("source=N/A")
        case "alt_text":
...
def _vertex_panel_title(vertex: Vertex, *, truncate: bool = True) -> str:
    ...
    match vertex.vertex_type:
        ...
        case VertexType.ASSET:
            asset_label: Final[str] = (
                vertex.file_name or url_file_name(vertex.storage.location) or "<firebase_storage_url>"
            )
            title_content = (
                f"[bold orange1]{markup_escape('ASSET')}[/bold orange1]"
                f" [bold #00aa00]{markup_escape(asset_label)}[/bold #00aa00]"
            )
