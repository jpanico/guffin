...
|---|---|---|
| `gfm_bullet.lua` | the semantic-bullet scaffold `Div` (a classified list item's body, stamped with `data-guffin-semantic` / `data-guffin-semantic-glyph`) | the Div's bare content with the bullet glyph prepended to the item's line — Markdown list syntax offers no marker control, so `- ⇒ text` (dash plus glyph) is the whole treatment |
...
| `gfm_code_source.lua` | a `Div` with a `code-source` class (the source attribution below a sourced code listing) | the Div's bare content — the already-*italic* attribution line with its GitHub link — since plain Markdown cannot shrink or grey the line and a raw `<div>` wrapper would be noise |
