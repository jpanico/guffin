...
pypandoc.convert_text(json_str, "gfm", format="json", extra_args=[
    "--wrap=none",
    f"--lua-filter={gfm_dir / 'gfm_bullet.lua'}",
    f"--lua-filter={gfm_dir / 'gfm_callout.lua'}",
    f"--lua-filter={gfm_dir / 'gfm_quote.lua'}",
    f"--lua-filter={gfm_dir / 'gfm_code_source.lua'}",
...
|---|---|---|
| `gfm_bullet.lua` | the classification scaffold `Div` (a classified list item's body, stamped with `data-guffin-marker-glyph`) | the Div's bare content with the marker glyph prepended to the item's line — Markdown list syntax offers no marker control, so `- ⇒ text` (dash plus glyph) is the whole treatment |
...
| `gfm_caption.lua` | a `Div` with a `caption` class (the caption below an image or code listing — the block's own nested content — or the `code-source` attribution below a sourced listing) | the Div's bare content — the caption's paragraphs, or the already-*italic* attribution line with its GitHub link — since plain Markdown cannot shrink or grey the text and a raw `<div>` wrapper would be noise |
