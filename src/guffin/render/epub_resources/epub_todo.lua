-- epub_todo.lua
-- Lua filter for the EPUB (XHTML) output path.
...
-- glyph that does NOT open its block is wrapped in an inline-styled span boosting it 20%
-- (line-height pinned so the taller glyph cannot inflate the line box).  The wrapped DONE
-- glyph is displayed as U+2611 BALLOT BOX WITH CHECK, matching the checkmark the writer's
-- native task-list checkboxes render with; U+2612 stays confined to the AST, where it is the
-- task-list convention the writer recognizes.
-- A glyph that IS the block's first inline is deliberately left bare: on a list item that
-- leading Str is what the writer's task-list conversion turns into a real
-- <input type="checkbox"/> element, which a wrapper would defeat.
