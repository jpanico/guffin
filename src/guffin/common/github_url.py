...
- :data:`RawGitHubUrl` — a validated ``raw.githubusercontent.com`` URL string (one that
  parses via :func:`parse_raw_github_url`); Pydantic enforces it wherever the annotation
  appears, the text kept verbatim.
- :class:`GitHubFileRef` — a raw GitHub file URL decomposed into its parts (owner, repo,
  ref, path, optional :class:`~guffin.common.line_range.LineRange`).
- :func:`verified_commit_sha` — return a string after verifying it is a full 40-hex commit SHA.
- :func:`verified_raw_github_url` — return a string after verifying it parses as a raw
  GitHub URL.
- :func:`parse_raw_github_url` — decompose a ``raw.githubusercontent.com`` URL into a
  :class:`GitHubFileRef`.
...
class GitHubFileRef(BaseModel):
    """A raw GitHub file URL decomposed into its parts.

    The decomposed form of a ``raw.githubusercontent.com`` URL (see
    :func:`parse_raw_github_url`): which file, in which repository, at which ref, optionally
...
@validate_call
def parse_raw_github_url(url: str) -> GitHubFileRef:
    """Decompose a ``raw.githubusercontent.com`` URL into a :class:`GitHubFileRef`.

    The URL path must be ``/<owner>/<repo>/<ref>/<file-path>``, where ``<ref>`` is a single
    segment (a branch, tag, or commit SHA) or the three-segment qualified form
    ...
    Args:
        url: The candidate raw GitHub URL.

    Returns:
        The decomposed reference.

    Raises:
        ValueError: If *url* is not https, is not hosted on ``raw.githubusercontent.com``,
            carries a query string, has too few or empty path segments, or carries a fragment
            that is not a line range.
    """
    split_url: Final = urlsplit(url)
    if split_url.scheme != "https":
        raise ValueError(f"raw GitHub URL {url!r} must use the https scheme")
    if split_url.hostname != RAW_GITHUB_HOST:
        raise ValueError(f"raw GitHub URL {url!r} is not hosted on {RAW_GITHUB_HOST}")
    if split_url.query:
        raise ValueError(f"raw GitHub URL {url!r} must not carry a query string")
    segments: Final[list[str]] = split_url.path.strip("/").split("/")
    if len(segments) < 4 or any(not segment for segment in segments):
        raise ValueError(f"raw GitHub URL {url!r} needs a /<owner>/<repo>/<ref>/<file-path> path")
    qualified: Final[bool] = segments[2] == _REF_QUALIFIER_SEGMENT and segments[3] in _REF_KIND_SEGMENTS
    ref_length: Final[int] = 3 if qualified else 1
    if len(segments) < 3 + ref_length:
        raise ValueError(f"raw GitHub URL {url!r} has a qualified ref but no file path")
    return GitHubFileRef(
        owner=segments[0],
        repo=segments[1],
        ref="/".join(segments[2 : 2 + ref_length]),
        path="/".join(segments[2 + ref_length :]),
        line_range=_parsed_line_range(split_url.fragment),
    )


def _checked_raw_github_url(url_text: str) -> str:
    """Return *url_text* after checking it parses as a raw GitHub URL.
    ...
    Returns:
        *url_text*, unchanged — the verbatim text, with the parse result discarded (derive
        it on demand via :func:`parse_raw_github_url`).

    Raises:
        ValueError: If *url_text* is not a parseable raw GitHub URL (see
            :func:`parse_raw_github_url`).
    """
    parse_raw_github_url(url_text)
    return url_text


type RawGitHubUrl = Annotated[str, AfterValidator(_checked_raw_github_url)]
"""A validated ``raw.githubusercontent.com`` URL string, kept verbatim.

The annotation carries the full grammar check — the string parses via
:func:`parse_raw_github_url` — so Pydantic enforces it wherever the type appears, while the
text itself stays exactly as authored (the decomposed form is derived on demand, not
stored).  To validate imperatively, use :func:`verified_raw_github_url`.
"""


@validate_call
def verified_raw_github_url(url_text: str) -> RawGitHubUrl:
    """Return *url_text* after verifying it parses as a raw GitHub URL.

    The imperative form of :data:`RawGitHubUrl`: applies the same check the annotation
    carries.
    ...
    Returns:
        *url_text*, unchanged, established as a :data:`RawGitHubUrl`.

    Raises:
        ValueError: If *url_text* is not a parseable raw GitHub URL (see
            :func:`parse_raw_github_url`).
    """
    return _checked_raw_github_url(url_text)
...
def blob_github_url(file_ref: GitHubFileRef, commit_sha: CommitSha | None = None) -> str:
    ...
    """
    ref: Final[str] = commit_sha if commit_sha is not None else file_ref.ref
    base: Final[str] = f"https://{_GITHUB_HOST}/{file_ref.owner}/{file_ref.repo}/blob/{ref}/{file_ref.path}"
