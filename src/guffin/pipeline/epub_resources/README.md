...
| Filter | Rewrites |
|---|---|
| `epub_callout.lua` | prepends a `callout-label` (the shared SVG icon from `../callout_icons/` + the type word) to each `div.callout` |
...
- **callout** — the GFM filter emits GitHub blockquote-alert syntax, which is not valid XHTML, so it
  is not reused. The callout `<div class="callout callout-TYPE">` is styled by `epub.css`, and
  `epub_callout.lua` (above) prepends the icon + label indicator.
